\begin{problem}
  ...
  \begin{enumroman}
    ...
    \item A specular surface perfectly reflects \emph{radiance} in the
      ...
      %   \[
      %     \int_{H^2} f_r(\hat{\bv}_i, \hat{\bv}_r) \cos\theta \d\vec{\omega}_i = 1.
      %   \]
      %   Substituting in the known values gives:
      %   \begin{align*}
      %     \int_{H^2} k \delta(\hat{\bv}_r - \hat{\bv}_s) \cos\theta_i \d\vec{\omega}_i &= 1 \\
      %     k \int_{H^2} \delta(\hat{\bv}_r - \hat{\bv}_s) \cos\theta_i \d\vec{\omega}_i &= 1 \\
      %     k \cos\theta_i &= 1 \\
      %     k &= \frac{1}{\cos\theta_i}.
      %   \end{align*}
